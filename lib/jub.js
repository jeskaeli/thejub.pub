...
function JubDJ(config, gapi, chat, db) {
  ...
  this.enqueue_video = function(video_obj, callback) {
    var user = video_obj.user,
        playlist = video_obj.playlist;
    if (!playlist) { return console.error('video submitted without playlist', video_obj); }
    if (user && playlist) {
      ...
      gapi.video_specs(video_obj, function(obj) {
        if (obj.title && obj.duration && user) {
          async.series([
            function(done) {
              db.updateOrCreatePlaylist(user, playlist, function(playlist, callback) {
                playlist.videos.unshift(obj); //TODO
                callback(done);
              });
            },
            function(done) {
