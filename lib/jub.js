...
function JubDJ(config, gapi, chat, db) {
  ...
  // TODO Playlists need a concept of 'order'
  this.shuffle = function(user, playlist_name, callback) {
    console.log('shuffling for ', user, playlist_name);
    jub.update_playlist(user, playlist_name, function(playlist) {
      console.log('shuffle commencing');
      console.log('old order', playlist);
      jub_util.knuth_shuffle(playlist.videos);
      console.log('new order', playlist);
    });
  }
