...
function JubDJ(config, gapi, chat, db) {
  ...
  this.update_playlist = function(user, playlist_name, update_fn) {
    if (user && playlist_name) {
      async.series([
        // Fetch/create the playlist and call the update function
        function(done) {
          db.updateOrCreatePlaylist(user, playlist_name, function(playlist, callback) {
            update_fn(playlist);
            callback(done);
          });
        },
        ...
        function(done) {
          console.log(user, "added video:", obj);
  ...
  this.enqueue_video = function(user, playlist_name, video, callback) {
    console.log('enqueue video', video);
    if (!(playlist_name && user && video)) {
      return console.error('insufficient args to add video');
    }
    gapi.video_specs(video, function(obj) {
      if (obj.title && obj.duration) {
        jub.update_playlist(user, playlist_name, function(playlist) {
          console.log('enqueue video callback called');
          playlist.videos.unshift(video);
        });
      } else {
        console.log('failed to find info for video', video);
      }
    });
  }
  ...
  this.save_sandbox_as = function(user, playlist_name, whisper_to_client) {
    db.updateOrCreatePlaylist(user, 'sandbox', function(playlist, save) {
      playlist.name = playlist_name;
      save(whisper_to_client);
    });
  }
  ...
  this.update_playlist = function(user, update) {
    db.updateOrCreatePlaylist(user, update);
  }
