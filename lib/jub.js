...
function JubDJ(config, gapi, chat, db) {
  ...
  var rotate_videos = function() {
    ...
    if (video_done() || Object.keys(video_state).length == 0) {
      ...
      // TODO when there's only one user and he's trying to loop a single video,
      // this check prevents him from doing that.
      if (sum_videos_queued() == 0) {
        ...
        // Use async series to keep the DB interactions synchronous with the
        // rest of the flow.
