...
function Socketeer(jub, chat, config, io) {
  ...
  io.on('connection', function(socket) {
    ...
    socket.on('send to bottom', function(user, indices, callback) {
      jub.send_to_bottom(user, indices, callback);
    });
    ...
    // Client wants to save their sandbox
    socket.on('save sandbox as', function(user, playlist_name) {
      console.log('client wishes to save sandbox as playlist', playlist_name);
      jub.save_sandbox_as(user, playlist_name, function() {
        emit_playlists_for(user);
      });
    });

    socket.on('user update preferences', function(user, prefs) {
      jub.user_update_preferences(user, prefs);
    });
