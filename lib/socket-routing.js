...
function Socketeer(jub, chat, config, io) {
  ...
  // TODO periodically trim sockets
  ...
  // TODO hopefully get rid of this at some point
  ...
  io.on('connection', function(socket) {
    ...
    // for sending the user's data back to him.
    socket.on('user loaded', function(user_info, callback) {
      // Validate, modify (if necessary), store and return the user's info
      jub.user_loaded(user_info, callback);

      add_user_socket(user_info.name, socket);

      // Send the user a welcome message in chat
      chat.welcome(user_info.name);
