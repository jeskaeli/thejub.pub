...
function JubDJ(config, gapi, chat) {
  ...
  this.user_loaded = function(user_info, callback) {
    user_info = user_info || {}
    if (!user_info.name) {
      user_info.name = chat.gen_username()
    }
    if (!user_info.color) {
      user_info.color = chat.color_for(user_info.name, user_info.color);
    }
    user_info.dj = (dj_sched.indexOf(user_info.name || '') > -1);

    console.log('user loaded, info:', user_info);
    callback(user_info);
  }
